...
void main_update_title_text()
{
	...
	rect.top = rect.left = 0; rect.right = width * 2; rect.bottom = Skin.Object[SongtitleText].h;
...
{
	/** CHANGED - improved options dialog */
	// was previously set to CreateDialog
...
main_windowproc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	/** CHANGED - BUGFIX - using -/+showplaylist on commandline messes with toggle playlist **/
...
/** CHANGED - BUGFIX - using -h would kill existing instances **/
...
int    *cmdline_get_argument(char *arg,HWND hWnd)
{
    ...
    if (_stricmp(arg, "help") == 0 || arg[0] == '?'
        ...
        cmdline_usage(hWnd);	/** CHANGED - BUGFIX - using -h would kill existing instances **/
        ...
        HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	HWND hWndCoolPlayer = NULL;	/** CHANGED - BUGFIX - using -h would kill existing instances **/
    ...
    cmdline_parse_options(__argc,__argv, hWndCoolPlayer);	/** CHANGED - BUGFIX - using -h would kill existing instances **/
