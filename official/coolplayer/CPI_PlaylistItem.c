...
void CPLI_CalculateLength_WAV(CPs_PlaylistItem* pItem);	/** CHANGED - BUGFIX - length of WAV files not showing in playlist */
...
void CPLI_CalculateLength(CP_HPLAYLISTITEM hItem)
{
	...
	/** CHANGED - BUGFIX - length of WAV files not showing in playlist*/
...
{
	/** CHANGED - BUGFIX - length of WAV files not showing */
	// TODO - reformat and check - move all this to separate function
	// as CPI_Player_CoDec_WAV does this aswell
...
void CPLI_WriteTag_OGG(CPs_PlaylistItem* pItem, HANDLE hFile)
{
    ...
    // Pack data into block
    {
        ...
        // Build the CRC
        {
            unsigned int iCRC;
            int iByteIDX;
			extern const ogg_uint32_t crc_lookup[256];	/** CHANGED - found in framing.c (ogg) */
